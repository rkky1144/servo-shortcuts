...
namespace servoCalculation {
    ...
    export function clampOpen(open: boolean, t3: number) {
        if (open) {
            SuperBit.Servo(SuperBit.enServo.S3, 135 + t3)
        } else {
            SuperBit.Servo(SuperBit.enServo.S3, 10 + t3)
        }
    }
    ...
    export function clampAt(first_x: number, first_y: number, forward: boolean, t1: number, t2: number) {
        if (forward) {
            for (let index = 0; index <= 100; index++) {
                ...
                SuperBit.Servo(SuperBit.enServo.S2, 90 + (Math.atan2(y, x) * 180 / 3.14159) - (Math.acos(Math.sqrt(x ** 2 + y ** 2) / 160) * 180 / 3.14159) + t2)
                basic.pause(5)
            }
        } else {
            for (let index2 = 100; index2 >= 0; index2--) {
                ...
                SuperBit.Servo(SuperBit.enServo.S2, 90 + (Math.atan2(y2, x2) * 180 / 3.14159) - (Math.acos(Math.sqrt(x2 ** 2 + y2 ** 2) / 160) * 180 / 3.14159) + t2)
                basic.pause(5)
            }
        }
    }
